...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedSittingInAnyVehicle(plyPed) then
			...
			if IsControlJustReleased(0, 159) then -- conducteur
				SetPedIntoVehicle(plyPed, plyVehicle, -1)
				Citizen.Wait(10)
			end

			if IsControlJustReleased(0, 161) then -- avant droit
			...
			if IsControlJustReleased(0, 162) then -- arriere gauche
				SetPedIntoVehicle(plyPed, plyVehicle, 1)
				Citizen.Wait(10)
			end

			if IsControlJustReleased(0, 163) then -- arriere gauche
