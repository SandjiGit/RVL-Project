         ...
         while open do 
            RageUI.IsVisible(mainMenu6,function() 
              ...
              RageUI.Separator("~h~↓ Véhicules ↓")
                ...
                for k,v in pairs(Config2.AmbuVehiculesAmbulance) do
...
Citizen.CreateThread(function()
  while true do 
      ...
      if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
          for k in pairs(Config2.Position.GarageVehicule) do 
              local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
              local pos = Config2.Position.GarageVehicule
              ...
              if dist <= 2.0 then 
                  wait = 0
                  Visual.Subtitle(Config2.TextGarageVehicule, 1)
         ...
         while open do 
            RageUI.IsVisible(mainMenu6,function() 
              ...
              RageUI.Separator("~h~↓ Véhicules ↓")
                ...
                for k,v in pairs(Config2.AmbuHelicoAmbulance) do
...
Citizen.CreateThread(function()
  while true do 
      local wait = 750
      if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
          for k in pairs(Config2.Position.GarageHeli) do 
              local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
              local pos = Config2.Position.GarageHeli
              local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, pos[k].x, pos[k].y, pos[k].z)
              ...
              if dist <= 2.0 then 
                  wait = 0
                  Visual.Subtitle(Config2.TextGarageHeli, 1)
                  if IsControlJustPressed(1,51) then
