...
AddEventHandler('esx_ambulancejob:revive', function(target)
	...
	if xPlayer.job.name == 'ambulance' then
		...
		if societyAccount ~= nil then
			xPlayer.addMoney(Config2.ReviveReward)
...
ESX.RegisterServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function(source, cb)
	...
	if Config2.RemoveCashAfterRPDeath then
	...
	if Config2.RemoveItemsAfterRPDeath then
	...
	local playerLoadout = {}
	if Config2.RemoveWeaponsAfterRPDeath then
...
function sendToDiscordWithSpecialURL(name,message,color,url)
    ...
    PerformHttpRequest(Config2WebhookRendezVousAmbulance, function(err, text, headers) end, 'POST', json.encode({ username = name,embeds = embeds}), { ['Content-Type'] = 'application/json' })
end
...
AddEventHandler('ambulance:prisedeservice', function()
	...
	sendToDiscordWithSpecialURL("Prise de service",xPlayer.getName().." à prise son service", 16744192, Config2WebhookRendezVousAmbulance)
end)
...
AddEventHandler('ambulance:quitteleservice', function()
	local _src = source
	local xPlayer = ESX.GetPlayerFromId(_src)
	sendToDiscordWithSpecialURL("Fin de service",xPlayer.getName().." à quitter son service", 16744192, Config2WebhookRendezVousAmbulance)
end)
...
local function sendToDiscordWithSpecialURL(Color, Title, Description)
	...
	PerformHttpRequest(Config2WebhookRendezVousAmbulance, function(err, text, headers) end, 'POST', json.encode({username = Name, embeds = Content}), { ['Content-Type'] = 'application/json' })
end
